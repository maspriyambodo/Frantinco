...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `brand_per_month` (IN `tahun` BIGINT, IN `brand_id` INT)  BEGIN
	SELECT 
		*
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `group_menu` ()  BEGIN
SELECT link FROM group_menu;
END$$

DROP PROCEDURE IF EXISTS `mt_country`$$
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `mt_country` (IN `param` VARCHAR(50), IN `country_id` INT, IN `kode_negara` VARCHAR(2), IN `nama_negara` VARCHAR(45), IN `bendera` VARCHAR(50), IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `mt_will_kab` (IN `param` VARCHAR(10), IN `kab_id` CHAR(2), IN `nama_kab` TINYTEXT, IN `lat` DOUBLE, IN `longtitut` DOUBLE, IN `user_login` INT)  BEGIN
	IF param = 'select' THEN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `mt_will_prov` (IN `param` VARCHAR(10), IN `prov_id` CHAR(2), IN `nama_prov` TINYTEXT, IN `lat` DOUBLE, IN `longtitut` DOUBLE, IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_app_select` ()  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_auth` (IN `usernama` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_active` (IN `id_menu` INT, IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_delete` (IN `id_menu` INT, IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_dir` ()  BEGIN 
SELECT 
  * 
FROM 
  sys_menu_select 
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_getorder` (IN `user_role` INT, IN `menu_grup` INT)  BEGIN 
SELECT 
  sys_menu_select.order_no, 
  sys_menu_select.nama_menu 
FROM 
  sys_menu_select 
WHERE 
  sys_menu_select.stat_menu = 1 
  AND sys_menu_select.role_id = user_role 
  AND sys_menu_select.id_group_menu = menu_grup 
GROUP BY 
  sys_menu_select.order_no 
ORDER BY
  sys_menu_select.id_group_menu ASC;
END$$

DROP PROCEDURE IF EXISTS `sys_menu_group`$$
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_group` (IN `param` VARCHAR(50), IN `id_grup` INT, IN `nama_group` VARCHAR(255), IN `deskripsi` VARCHAR(255), IN `no_order` INT, IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_group_reorder` (IN `old_id` INT, IN `old_order` INT, IN `new_id` INT, IN `new_order` INT)  BEGIN 
UPDATE 
  sys_menu_group 
SET 
  order_no = new_order 
WHERE 
  sys_menu_group.id = old_id;
...
SET 
  order_no = old_order 
WHERE 
  sys_menu_group.id = new_id;
END$$

DROP PROCEDURE IF EXISTS `sys_menu_group_select`$$
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_group_select` ()  BEGIN
	SELECT * 
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_insert` (IN `parent` INT, IN `nama_menu` VARCHAR(50), IN `link_menu` VARCHAR(255), IN `no_order` INT, IN `gr_menu` INT, IN `ico_menu` VARCHAR(50), IN `desc_txt` VARCHAR(255), IN `user_login` INT)  BEGIN
	DECLARE
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_order` (IN `old_id` INT, IN `old_order` INT, IN `new_id` INT, IN `new_order` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_select` (IN `user_role` INT)  BEGIN 
SELECT 
  * 
FROM 
  sys_menu_select 
...
ORDER BY 
  sys_menu_select.order_no ASC,
	sys_menu_select.group_order ASC;
END$$

DROP PROCEDURE IF EXISTS `sys_menu_update`$$
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_menu_update` (IN `parent` INT, IN `menu` VARCHAR(50), IN `location` VARCHAR(255), IN `nomor_order` INT, IN `grup` INT, IN `icon_menu` VARCHAR(50), IN `user_login` INT, IN `id_menu` INT, IN `desc_txt` VARCHAR(255), OUT `menu_nama` VARCHAR(50))  BEGIN
SELECT sys_menu.nama INTO menu_nama FROM sys_menu WHERE sys_menu.id = id_menu;
UPDATE sys_menu
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_mt_country` (IN `param` VARCHAR(50), IN `country_id` INT, IN `kode_negara` VARCHAR(2), IN `nama_negara` VARCHAR(45), IN `bendera` VARCHAR(50), IN `user_login` INT)  BEGIN
IF param = 'select' THEN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_permision_update` (IN `id_role` INT, IN `menu_id` INT, IN `lihat` INT, IN `buat` INT, IN `baca` INT, IN `ubah` INT, IN `hapus` INT, IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_permission_select` (IN `permisi_id` INT)  BEGIN 
SELECT 
  `sys_menu_select`.`id_menu`, 
  `sys_menu_select`.`nama_menu`,
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_roles_insert` (IN `nama` VARCHAR(30), IN `deskripsi` VARCHAR(255), IN `group_parent` INT, IN `user_id` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_roles_select` (IN `grup_id` INT)  BEGIN
IF grup_id = 0 THEN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_roles_update` (IN `id_grup` INT, IN `nama_grup` VARCHAR(30), IN `des_grup` VARCHAR(255), IN `user_login` INT, IN `id_parent` INT)  BEGIN
UPDATE sys_roles 
SET
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_users_insert` (IN `user_name` VARCHAR(100), IN `pass_word` VARCHAR(100), IN `id_role` INT, IN `avatar` VARCHAR(100), IN `status_active` INT, IN `user_login` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_users_select` (IN `param` VARCHAR(50), IN `user_id` INT, IN `panjang` INT, IN `mulai` INT)  BEGIN
...
CREATE DEFINER=`maspriya`@`localhost` PROCEDURE `sys_users_stat` (IN `id_user` INT, IN `user_login` INT, IN `stat_active` INT)  BEGIN
UPDATE sys_users
