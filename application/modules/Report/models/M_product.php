...
class M_product extends CI_Model {
    ...
    public function chartdiv($tahun) {
        $exec = $this->db->select('TRIM(LEADING 0 FROM mt_month.angka) AS angka,
	`mt_month`.`bulan`,
    ...
    public function chartdiv_a($tahun) {
        $exec = $this->db->select('v_transaction.nama_kategorisub,SUM(v_transaction.qty) AS qty,v_transaction.kode_product')
                ->from('mt_product')
                ->join('v_transaction', 'mt_product.id = v_transaction.id_product ')
    ...
    public function not_exists($tahun) {
        $exec = $this->db->select('v_transaction.id_product,v_transaction.nama_brand,v_transaction.nama_kategori,v_transaction.nama_produk,v_transaction.nama_kategorisub,mt_product.kd_produk AS kode_product,NOW( ) AS tr_date,0 AS `JANUARI`,0 AS `FEBRUARI`,0 AS `MARET`,0 AS `APRIL`,0 AS `MEI`,0 AS `JUNI`,0 AS `JULI`,0 AS `AGUSTUS`,0 AS `SEPTEMBER`,0 AS `OKTOBER`,0 AS `NOVEMBER`,0 AS `DESEMBER` ')
                ->from('mt_product')
                ->join('v_transaction', 'mt_product.id = v_transaction.id_product', 'LEFT')
                ->where('`mt_product`.`id` NOT IN', '( SELECT v_transaction.id_product FROM v_transaction WHERE YEAR ( tr_date ) = ' . $tahun . ' )', false)
                ->group_by('mt_product.id')
                ->order_by('mt_product.nama ASC')
                ->get()
                ->result();
        return $exec;
    }
    ...
    public function dt_table($tahun) {
        $exec = $this->db->select('v_transaction.id_product,v_transaction.nama_brand,v_transaction.nama_kategori,v_transaction.nama_produk,v_transaction.nama_kategorisub,
	v_transaction.kode_product,
